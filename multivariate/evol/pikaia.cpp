...
PikaiaSearch::PikaiaSearch(int np, int ngen, int nd, double pcross, int imut, // @suppress("Class members should be properly initialized")
...
void PikaiaSearch::init(multivariate f, const int n, double *guess,
	...
	// initialize memory
	_ph = std::vector<double>(2 * n);
	_oldph = std::vector<double>(_np * n);
	_newph = std::vector<double>(_np * n);
	_gn1 = std::vector<int>(n * _nd);
	_gn2 = std::vector<int>(n * _nd);
	...
	for (int ip = 1; ip <= _np; ip++) {
		double *oldph_ptr = row_ptr(ip, n, &_oldph[0]);
		for (int k = 1; k <= n; k++) {
			oldph_ptr[k - 1] = Random::get(0., 1.);
		}
		_fitns[_np - 1] = f(oldph_ptr);
	}
	_fev = _np;
...
void PikaiaSearch::iterate() {
	...
	for (int ip = 1; ip <= _np / 2; ip++) {
		...
		if (_irep == 1) {
			...
			stdrep(_f, _n, _n, _np, _irep, _ielite, &_ph[0], &_oldph[0],
	...
	if (_irep == 1) {
		newpop(_f, _ielite, _n, _n, _np, &_oldph[0], &_newph[0], &_ifit[0],
...
multivariate_solution PikaiaSearch::optimize(multivariate f, const int n,
	...
	double *oldph_ptr = row_ptr(_ifit[_np - 1], n, &_oldph[0]);
	std::copy(oldph_ptr, oldph_ptr + n, &x[0]);
	return {x, _fev, true};
}
...
void PikaiaSearch::cross(int n, int nd, double pcross, int *gn1, int *gn2) {
	...
	if (Random::get(0., 1.) < pcross) {
		...
		int ispl = (int) (Random::get(0., 1.) * n * nd) + 1;
		...
		if (Random::get(0., 1.) < 0.5) {
			...
			ispl2 = (int) (Random::get(0., 1.) * n * nd) + 1;
		...
		for (i = 1; i <= n; i++) {
			for (j = 1; j <= nd; j++) {
				if (Random::get(0., 1.) < pmut) {
					...
					// Construct integer
					loc = (i - 1) * nd + j;
					inc = (int) std::round(Random::get(0., 1.)) * 2 - 1;
					ist = (i - 1) * nd + 1;
					gn[loc - 1] += inc;
...
void PikaiaSearch::stdrep(multivariate ff, int ndim, int n, int np, int irep,
		...
		for (i = np; i >= 1; i--) {
			if (fit > fitns[ifit[i - 1] - 1]) {
				...
				if (irep == 3) {
					...
					i1 = (int) (Random::get(0., 1.) * np) + 1;
				} else {
					i1 = (int) (Random::get(0., 1.) * (np - 1)) + 1;
				}
				if1 = ifit[i1 - 1];
				fitns[if1 - 1] = fit;
...
void PikaiaSearch::newpop(multivariate ff, int ielite, int ndim, int n, int np,
