...
void Praxis::init(multivariate f, const int n, double *guess, double *lower,
		double *upper) {
	// nothing to do here
}
...
multivariate_solution Praxis::optimize(multivariate f, const int n,
		double *guess, double *lower, double *upper) {
	std::vector<double> x(guess, guess + n);
	const int fev = praxis(n, &x[0], f);
	return {x, fev, false};
}

int Praxis::praxis(int n, double *x, multivariate f) {
	bool fk, illc;
...
double Praxis::flin(int n, int jsearch, double l, multivariate f, double *x,
		int &nf, double *v, double *q0, double *q1, double &qd0, double &qd1,
...
void Praxis::minny(int n, int jsearch, int nits, double &d2, double &x1,
		double &f1, bool fk, multivariate f, double *x, double t, double h,
...
void Praxis::quad(int n, multivariate f, double *x, double t, double h,
		double *v, double *q0, double *q1, int &nl, int &nf, double dmin,
