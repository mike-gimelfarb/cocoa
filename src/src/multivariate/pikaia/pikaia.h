...
#ifndef MULTIVARIATE_EVOL_PIKAIA_H_
#define MULTIVARIATE_EVOL_PIKAIA_H_
...
public:
	PikaiaSearch(int np, int ngen, int nd, double pcross=0.85, int imut=2, double pmut=0.005,
			double pmutmn=0.0005, double pmutmx=0.25, double fdif=1., int irep=1, int ielite=0);
...
private:
	...
	void stdrep(multivariate ff, int ndim, int n, int np, int irep, int ielite,
			double *ph, double *oldph, double *fitns, int *ifit, int *jfit,
			int &nnew);

	void newpop(multivariate ff, int ielite, int ndim, int n, int np,
			double *oldph, double *newph, int *ifit, int *jfit, double *fitns,
			int &nnew);
};

#endif /* MULTIVARIATE_EVOL_PIKAIA_H_ */
