...
template<typename T> GlobalBrentSearch<T>::GlobalBrentSearch(int mfev,
		double tol, double bound_on_hessian) {
	_mfev = mfev;
	_tol = tol;
	_boundhess = bound_on_hessian;
}

template<typename T> solution<T> GlobalBrentSearch<T>::optimize(
	...
	if (M <= T(0.) || a >= b) {
		std::cerr << "Warning [Global Brent]: bounding interval is ill defined."
				<< std::endl;
		return {x, fev, false};
	}
	if (sc <= a || sc >= b) {
